...
PINK = (255, 192, 203)  # Розовый цвет для еды после времени
...
def gameLoop():
    ...
    food_eaten = 0
    OPO = 5  # Порог для увеличения уровня
    snake_speed = 15  # Начальная скорость змейки
    ...
    food_color = RED  # Начальный цвет еды
    food_timer = pygame.time.get_ticks()  # Время генерации еды
    food_lifetime = 5000  # 5 секунд
    food_time = 4000  # Время до смены цвета еды
    ...
    while game:
        ...
        # Рисуем еду
        i = pygame.draw.rect(screen, food_color, [foodx, foody, snake_block, snake_block])

        # Проверка на время жизни еды
        if pygame.time.get_ticks() - food_timer > food_time:  # Если прошло больше 3 секунд
            food_color = PINK  # Меняем цвет еды на розовый
            if pygame.time.get_ticks() - food_timer > food_lifetime:  # Если прошло больше 5 секунд
                foodx, foody = generate_food()  # Генерируем новую еду
                food_timer = pygame.time.get_ticks()  # Обновляем таймер
                food_color = RED  # Сбрасываем цвет обратно на красный
        ...
        if x1 == foodx and y1 == foody:
            ...
            Length_of_Snake += eat  # Увеличиваем длину змейки
            food_eaten += eat  # Увеличиваем количество съеденной пищи
            # Увеличиваем уровень каждые 5 съеденных кусочков пищи
            if Length_of_Snake-1 >= OPO:
                level += 1
